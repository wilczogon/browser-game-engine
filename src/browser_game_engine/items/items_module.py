...
from browser_game_engine import EngineModule, BadRequest, db, error_handling, app
from .character_to_item import CharacterToItem
from .models import ItemCategory
from sqlalchemy import and_


class ItemsModule(EngineModule):
    ...
    def add_endpoints(self):
        @app.route(self.engine.root_path + "/characters/<character_id>/eat", methods=['POST'])
        @error_handling
        ...
        def eat(user, character):
            ...
            for stat in item_definition.modifications:
                character.__setattr__(stat, self.engine.characters.character_class.__dict__[stat] + item_definition.modifications[stat])
                db.session.commit()
                character_cls = self.engine.characters.character_class
            ...
            return jsonify(self.engine.characters.get_character_json(user, character.id))
